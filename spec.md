...
- Mức prototype nhắm tới: [ ] Sketch [x] Mock [ ] Working — `prototype/index.html` (trang HTML tĩnh, mở bằng trình duyệt, bấm thử được cả 4 đường đi):
  ...
  | Rẽ nhánh 4 đường (happy / ② / ① / correction) + ngưỡng quyết định | **Thật** — logic rẽ nhánh chạy trên điểm khớp | Giữ logic, thay nguồn điểm |
  ...
  | Gửi TA, ghi correction log | **Giả lập** — hiện form/trace, không gửi | Ghi log correction ra file để đưa vào golden set |
...
- §4b. Nguyên tắc đã áp dụng (≥4 — HAX/PAIR, xem guide):
  ...
  | **HAX G9 — Support efficient correction** | Dưới mỗi câu trả lời: nút "Nguồn không đúng" → hiện danh sách các đoạn khác trong bài, học viên chọn 1 đoạn → AI trả lời lại chỉ theo đoạn đó (nhãn "Đã trả lời lại theo nguồn bạn chọn"), câu trả lời cũ bị gạch mờ; nút "Sửa câu hỏi" đưa câu cũ về ô nhập để sửa. Correction được ghi vào trace để đưa vào golden set. |
...
- **Correction (user sửa)** — *"Vì sao cần chia nhỏ tài liệu?"* → có câu trả lời kèm Slide 2 · đoạn 1 → học viên bấm "Nguồn không đúng" → chọn đoạn khác (vd. Slide 2 · đoạn 2 Overlap) → câu trả lời cũ bị gạch mờ, AI trả lời lại chỉ theo đoạn đã chọn, nhãn "Đã trả lời lại theo nguồn bạn chọn"; cặp (câu hỏi, nguồn sai → nguồn đúng) ghi vào correction log để bổ sung golden set. Hoặc "Sửa câu hỏi" → câu cũ về ô nhập để sửa và hỏi lại.
